...
> [rke2-cluster/rke2-addons-istio-argocd-headlamp.md](../../docs/runbooks/rke2-addons-istio-argocd-headlamp.md).
...
| Deploy mechanism | ArgoCD GitOps (planned) | Web UI WSO2 cards (`ansible/k8s/wso2.yml` renders + applies the repo) or `kubectl apply -f` (her steps) |
...
- **Production:** point WSO2 APIM/IS JDBC URLs at the **AG primary node** (or the Pacemaker VIP /
  ...
  [`ansible/db/mssql_wso2_db.yml`](../../ansible/db/mssql_wso2_db.yml) (UI card **"7b · WSO2 DB user +
...
- **Credentials are now variables, not hardcoded:** the DB user/password in each `deployment.toml`
  ...
  [k8s_wso2.yml](../../ansible/k8s/wso2.yml). They **must match** the login created by `mssql_wso2_db.yml`.
...
[ansible/k8s_wso2.yml](../../ansible/k8s/wso2.yml)); ztunnel handles mTLS at L4 with **no
