...
Recommended: use the Python web UI to trigger `ansible/k8s/rke2_cluster.yml`, which installs RKE2
...
The web UI / `ansible/k8s/rke2_cluster.yml` performs the following. Manual equivalent below.
...
`ansible/k8s/rke2_cluster.yml` does 1a–1d for you and writes the kubeconfig to `$WORK_DIR/kubeconfig`.
