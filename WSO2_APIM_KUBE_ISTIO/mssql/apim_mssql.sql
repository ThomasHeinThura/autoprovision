-- WSO2 API Manager — MSSQL bootstrap for the SQL Server Always On AG (WSO2AM_DB).
...
--   sqlcmd -S <primary> -U sa -P '<sa-pw>' -C -b -i apim_mssql.sql \
--     -v WSO2_USER=wso2carbon -v WSO2_PW='<db-pw>' -v WSO2_SID=0x57534F3243415242000000000000ABCD
-- Use the IDENTICAL WSO2_SID on every node. CREATE LOGIN sets it explicitly; CREATE USER FOR LOGIN
-- then inherits it, so the replicated database user maps to the same-SID login on any primary.
IF EXISTS (SELECT * FROM sys.databases WHERE name = 'WSO2AM_DB')
BEGIN
    ALTER DATABASE WSO2AM_DB SET SINGLE_USER WITH ROLLBACK IMMEDIATE;
